...
2. **Shacket vs overshirt: what's the difference.** Fit: High (Ghillie sells overshirts directly) · Demand: Medium-High (The Modest Man, Moss, Gentleman's Gazette, Burrows & Hare all cover it; strong UK menswear search term) · Winnability: Medium (Barbour/ASOS content ranks here, but no field-guide-toned independent take spotted). Strong next pick if gilet post performs.
...
- Live blog (https://ghillieuk.com/blogs/news) and sitemap.xml returned HTTP 403 via WebFetch and via curl through the session's egress proxy this run. The proxy status endpoint confirmed `connect_rejected` / organisation policy denial for `ghillieuk.com:443`, not a transient error. Inventory and competitor research were built entirely from WebSearch snippets instead. If direct fetching is still blocked next run, keep using WebSearch as the fallback and note it here again; if it's unblocked, do a full direct-fetch pass to correct/complete the covered-log inventory (in particular, pin down the exact title/slug of the existing garment-care post referenced above).
- research/competitors.md does not exist yet; no owner-pinned extra competitors on file. Created it as an empty placeholder this run; check it each future run.
